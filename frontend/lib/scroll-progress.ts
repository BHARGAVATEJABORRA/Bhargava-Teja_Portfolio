 ...
 * canonical driver (it eases the native scroll position itself, per the
 * adaline architecture); the native scroll + resize listeners cover reduced
 * motion, the test harness, and the window before the Lenis provider has
 * mounted. Callbacks must be pure functions of the current scroll position,
 * so a double fire in one frame is a no-op.
 */
